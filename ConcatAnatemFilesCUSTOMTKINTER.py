...
# Dicionários para mapeamento de colunas
dict_UG = {
...
def ComparaUG(df_BDAna, df_BDT, dict_BD=dict_UG, init_key_UG=3):
    # Função para comparar e ajustar valores entre dois DataFrames
...
class FileSelector(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        # Switch para alternar entre modos claro e escuro
        self.appearance_mode = ctk.StringVar(value="light")

        self.label_tema = ctk.CTkLabel(self, text="Tema: Claro ")
        self.label_tema.grid(row=0, column=0, pady=10, padx=10, sticky="e")

        self.switch_theme = ctk.CTkSwitch(
            self, text="Escuro", command=self.toggle_appearance_mode
        )
        self.switch_theme.grid(row=0, column=1, pady=10, padx=(0, 0), sticky="ew")

        # Campos para selecionar os arquivos de entrada e o diretório de saída
        self.label_dat = ctk.CTkLabel(self, text="Arquivo BNT1 (.dat):")
        self.label_dat.grid(row=1, column=0, pady=10, padx=10, sticky="e")

        self.entry_dat = ctk.CTkEntry(self, width=400)
        self.entry_dat.grid(row=1, column=1, pady=10, padx=10, sticky="ew")

        self.button_dat = ctk.CTkButton(
            self, text="Buscar", command=self.select_dat_file
        )
        self.button_dat.grid(row=1, column=2, pady=10, padx=10, sticky="w")

        self.label_stb = ctk.CTkLabel(self, text="Arquivo DMAQ (.stb):")
        self.label_stb.grid(row=2, column=0, pady=10, padx=10, sticky="e")

        self.entry_stb = ctk.CTkEntry(self, width=400)
        self.entry_stb.grid(row=2, column=1, pady=10, padx=10, sticky="ew")

        self.button_stb = ctk.CTkButton(
            self, text="Buscar", command=self.select_stb_file
        )
        self.button_stb.grid(row=2, column=2, pady=10, padx=10, sticky="w")

        self.label_blt = ctk.CTkLabel(self, text="Arquivo BLT (.blt):")
        self.label_blt.grid(row=3, column=0, pady=10, padx=10, sticky="e")

        self.entry_blt = ctk.CTkEntry(self, width=400)
        self.entry_blt.grid(row=3, column=1, pady=10, padx=10, sticky="ew")

        self.button_blt = ctk.CTkButton(
            self, text="Buscar", command=self.select_blt_file
        )
        self.button_blt.grid(row=3, column=2, pady=10, padx=10, sticky="w")

        self.label_output = ctk.CTkLabel(self, text="Diretório de saída:")
        self.label_output.grid(row=4, column=0, pady=10, padx=10, sticky="e")

        self.entry_output = ctk.CTkEntry(self, width=400)
        self.entry_output.grid(row=4, column=1, pady=10, padx=10, sticky="ew")

        self.button_output = ctk.CTkButton(
            self, text="Selecionar", command=self.select_output_dir
        )
        self.button_output.grid(row=4, column=2, pady=10, padx=10, sticky="w")

        # Botão para iniciar o processamento dos arquivos
        self.button_process = ctk.CTkButton(
            self,
            text="Processar Arquivos",
            command=self.check_inputs_and_process,
            fg_color="green",
        )
        self.button_process.grid(row=5, column=0, columnspan=3, pady=20, padx=10)

    def toggle_appearance_mode(self):
        # Alterna entre os modos claro e escuro
        if self.appearance_mode.get() == "light":
    ...
    def adjust_entry_width(self, entry):
        # Ajusta automaticamente a largura do campo de entrada
        text_length = len(entry.get())
        new_width = max(400, text_length * 7)
        entry.configure(width=new_width)
        self.update_idletasks()
        self.geometry(f"{self.winfo_reqwidth()}x{self.winfo_reqheight()}")

    def check_inputs_and_process(self):
        # Verifica se os arquivos e o diretório foram selecionados antes de processar
        if (
            not self.entry_dat.get()
            or not self.entry_stb.get()
            or not self.entry_blt.get()
            or not self.entry_output.get()
        ):
            messagebox.showerror(
                "Erro", "Algum arquivo ou diretório não foi selecionado."
            )
        else:
            self.show_excel_selection()

    def show_excel_selection(self):
        # Seleção dos arquivos Excel para o processamento final
        self.label_ug = ctk.CTkLabel(self, text="Arquivo Export (UG).xls:")
        self.label_ug.grid(row=6, column=0, pady=10, padx=10, sticky="e")

        self.entry_ug = ctk.CTkEntry(self, width=400)
        self.entry_ug.grid(row=6, column=1, pady=10, padx=10, sticky="ew")

        self.button_ug = ctk.CTkButton(self, text="Buscar", command=self.select_ug_file)
        self.button_ug.grid(row=6, column=2, pady=10, padx=10, sticky="w")

        self.label_cs = ctk.CTkLabel(self, text="Arquivo Export (CS).xls:")
        self.label_cs.grid(row=7, column=0, pady=10, padx=10, sticky="e")

        self.entry_cs = ctk.CTkEntry(self, width=400)
        self.entry_cs.grid(row=7, column=1, pady=10, padx=10, sticky="ew")

        self.button_cs = ctk.CTkButton(self, text="Buscar", command=self.select_cs_file)
        self.button_cs.grid(row=7, column=2, pady=10, padx=10, sticky="w")
        ...
        self.button_final_process.grid(row=8, column=0, columnspan=3, pady=20, padx=10)
